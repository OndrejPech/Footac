...
# shot, foul, goal, penalty
no_passive_player_ids = {1, 3, 10, 11, 2}
...
class ActionForm(ModelForm):
    def __init__(self, *args, team_id, my_team_attack, action_type_id,  **kwargs):
        ...
        if my_team_attack:
            self.fields['active_player'].queryset = Player.objects.filter(teams=team_id)
            if action_type_id in no_passive_player_ids:
                self.fields['passive_player'].widget = forms.HiddenInput()  # Hide field
            else:
                self.fields['passive_player'].queryset = Player.objects.filter(teams=team_id)

        else:
            self.fields['active_player'].widget = forms.HiddenInput()  # Hide field
